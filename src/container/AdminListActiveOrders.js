...
import { active_Orders } from '../action/orderAction'


const AdminListActiveOrders = ({ history }) => {

    const dispatch = useDispatch()
    const { loadingActiveOrders, activeOrders, activeOrderError } = useSelector(state => state.ActiveOrders)
        ...
        <Fragment>
            ...
            <Row className="justify-content-md-center" style={{ padding: '34px' }}>
                <Col>
                    ...
                    <center>
                        <p className='heading'> Active Orders</p>
                    </center>
                    <br />

                    {
                        loadingActiveOrders ? <Loader />
                            :
                            activeOrderError ? <Message variant='danger'>{activeOrderError}</Message> :
                                activeOrders ?
                                    <Table striped bordered hover responsive className='table-sm'>
                                        ...
                                        <tbody>
                                            {
                                                activeOrders && activeOrders.map(items => {
                                                    return (
                                                        <tr key={items._id}>
...
export default AdminListActiveOrders
